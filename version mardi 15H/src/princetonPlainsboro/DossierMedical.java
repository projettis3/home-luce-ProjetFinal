...
class DossierMedical {
    ...
    private DefaultListModel modelActes;
    private DefaultListModel modelCoutActes;
    ...
    public DossierMedical() {
        ...
        modelActes = new DefaultListModel();
        modelCoutActes = new DefaultListModel();
    }
    ...
    public DefaultListModel getModelActes() {
        return modelActes;
    }

    public DefaultListModel getModelCoutActes() {
        return modelCoutActes;
    }

    public void ajouterFiche(FicheDeSoins fiche) {
        fiches.add(fiche);
        //Création des modeles pour remplir les tableaux
        //Patients
        if (modelPatient.contains(fiche.getPatient())) {
            int position = modelPatient.indexOf(fiche.getPatient());
            Object cout = modelCoutPatient.get(position);
            double prix = new Double(cout.toString());
            prix = prix + coutPatient(fiche.getPatient());
            modelCoutPatient.setElementAt(df.format(prix), position);
        }
        if (!modelPatient.contains(fiche.getPatient())) {
        ...
        //Medecins
        if (modelMedecin.contains(fiche.getMedecin().getIdentite())) {
            int position = modelMedecin.indexOf(fiche.getMedecin().getIdentite());
            Object cout = modelCoutMedecin.get(position);
            double prix = new Double(cout.toString());
            prix = prix + coutMedecin(fiche.getMedecin());
            modelCoutMedecin.setElementAt(prix, position);
        }
        if (!modelMedecin.contains(fiche.getMedecin().getIdentite())) {
            modelMedecin.addElement(fiche.getMedecin().getIdentite());
        ...
        //Actes
        for (int i = 0; i < fiche.getActes().size(); i++) {

            if (!modelActes.contains(fiche.getActes().get(i))) {
                modelActes.addElement(fiche.getActes().get(i));
                modelCoutActes.addElement(fiche.getActes().get(i).cout());
            }

            if (modelActes.contains(fiche.getActes().get(i))) {
                int position = modelActes.indexOf(fiche.getActes().get(i));
                Object cout = modelCoutActes.get(position);
                double prix = new Double(cout.toString());
                prix = prix + fiche.getActes().get(i).cout();
                modelCoutActes.setElementAt(prix, position);
            }
        }
...
//tri et affichage par dates
    ...
    public String fichePatient(Patient p) {
        String s = "> Fiche patient de " + p + "\n" + "-------------------------------------------------------" + "\n";
        double cout = 0;
        for (int i = 0; i < fiches.size(); i++) {
            if (fiches.get(i).getPatient() == p) {
                cout += fiches.get(i).coutTotal();
                for (int j = 0; j < fiches.get(i).getActes().size(); j++) {
                    s += fiches.get(i).getActes().get(j).getNomActe();
                }
            }
            s += "\n" + "_______________" + "\n" + "COUT TOTAL: " + cout;
        }
